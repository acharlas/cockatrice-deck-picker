...
export default function HistoryPage() {
  ...
  const handleReset = async () => {
    if (!confirm("Reset history? Previously assigned decks will be available again.")) {
      return
    }
      ...
      {history.length === 0 ? (
        <Card>
          <CardContent className="p-8 text-center">
            ...
            <p className="text-sm text-muted-foreground mt-2">
              Go to{" "}
              <a href="/assign" className="text-primary hover:underline">
                assignment
              </a>{" "}
              to start.
            </p>
        ...
        <div className="space-y-6">
          <div className="text-sm text-muted-foreground mb-4">
            {Object.keys(groupedHistory).length} games | {history.length} assignments
          </div>
                    ...
                    {gameData.assignments.map((entry) => (
                      <div key={entry.id} className="flex justify-between items-center p-2 bg-muted rounded">
                        <div>
                          ...
                          <span className="text-muted-foreground ml-2">{entry.deckName}</span>
                        </div>
                      </div>
                    ))}
                  </div>
